...
Current gaps relative to full Mode A/B/C: contact force and contact normal are not reliable exported fields; Mode B `groups/` artifacts and Mode C `trajectories/` artifacts are planned but not yet generated by the current Mode A implementation.
