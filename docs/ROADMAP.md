...
Current planning update: before model work, finish SOFA NJF dataset v1. The controlled liver surface dataset v1 now has a standard orchestration entry at `tissue_dataset_v0/scripts/run_liver_surface_controlled_v1.py` and a usage/interpretation document at `docs/CONTROLLED_SOFA_DATASET_V1.md`. The lightweight NJF orchestration layer, Mode A local perturbation generation, Mode B response basis groups, Mode C rollout trajectories, Mode A/B/C dataset validator, non-smoke K=24/T=10 demo, response-basis analysis script, rollout trajectory analysis script, Response Basis Batch v1, SOFA-free NJF dataset reader, Stage 1 cross-group response-basis analysis, Stage 2 action-linearity/superposition analysis, probe-depth diagnosis, micro-linearity probe, and preloaded incremental-action diagnostics are implemented. The current point-collision probe can be strict-linear only in an impractically small micro range; preload and zero-baseline subtraction do not make `0.05/0.1/0.2 mm` full-field responses linear enough. Stage 3 factorial data is blocked until a surface-collision or continuous/compliant contact prototype enlarges the realistic local linear range. See `tasks/0013-controlled-sofa-contact-dataset-v1.md` and `docs/sofa_njf_dataset_design.md`.
