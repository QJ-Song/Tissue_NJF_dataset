...
Current planning update: before model work, finish SOFA NJF dataset v1. The lightweight NJF orchestration layer, Mode A local perturbation generation, Mode B response basis groups, Mode C rollout trajectories, Mode A/B/C dataset validator, non-smoke K=24/T=10 demo, response-basis analysis script, rollout trajectory analysis script, Response Basis Batch v1, SOFA-free NJF dataset reader, Stage 1 cross-group response-basis analysis, Stage 2 action-linearity/superposition analysis, probe-depth diagnosis, and micro-linearity probe are implemented and validated. The original 0.05/0.1/0.2 mm grouped data is not suitable for strict local-Jacobian magnitude supervision because contactDistance and sparse point-collision geometry dominated depth semantics. The validated strict-local range for the current point-collision probe is conservatively `<= 0.01 mm`; the next priority is updating factorial dataset planning around geometry-aware contact points and this micro range, or moving to surface collision before increasing magnitudes. See `tasks/0013-controlled-sofa-contact-dataset-v1.md` and `docs/sofa_njf_dataset_design.md`.
