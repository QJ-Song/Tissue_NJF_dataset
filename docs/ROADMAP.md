...
Current planning update: before model work, finish SOFA NJF dataset v1. The lightweight NJF orchestration layer, Mode A local perturbation generation, and Mode A dataset validator are implemented. Remaining major work is Mode B response basis groups, Mode C multi-step rollout trajectories, group/trajectory validation, and non-smoke split/demo generation. See `tasks/0013-controlled-sofa-contact-dataset-v1.md` and `docs/sofa_njf_dataset_design.md`.
